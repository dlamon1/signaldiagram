<script lang="ts">
	...
	const setSignalDirection = (direction: DirectionObj) => {
		...
		let initSnapPointIndex = prevPanel.thisPanelsSnapPoints[direction.points[0].i[0]];
		...
		const getNexPanelObj = () => {
			...
			let initDirectionInstruction = direction.points[directionInstructionPosition];
		...
		for (let i = 1; i < selectedArray.length * 2 - 2; i++) {
			if (isSettingDestination) {
				...
				let destSnapPointIndex =
					nextPanel.thisPanelsSnapPoints[direction.points[directionInstructionPosition].i[1]];
				...
				let originSnapPointIndex =
					prevPanel.thisPanelsSnapPoints[direction.points[directionInstructionPosition].i[0]];
